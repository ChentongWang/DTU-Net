...
def dilateformer_tiny(pretrained=True, **kwargs):
    model = Dilateformer(depths=[2, 1, 2, 1], embed_dim=108, num_heads=[3, 6, 12, 24], **kwargs)
...
class multiStageUnmixing(nn.Module):
    def __init__(self):
        ...
        self.spectral = nn.Sequential(
            ...
            ChannelAttention(in_planes=222, ratio=2),
            ...
            ChannelAttention(in_planes=220, ratio=2),
            ...
            ChannelAttention(in_planes=108, ratio=2),
            ...
            nn.LeakyReLU(0.2),
            ChannelAttention(in_planes=52, ratio=2),
            nn.MaxPool3d((2, 1, 1)),
            ...
            ChannelAttention(in_planes=24, ratio=2),
            ...
            ChannelAttention(in_planes=10, ratio=2),
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(8, 8 // 2, kernel_size=1, stride=1, padding=0),
            nn.ReLU(inplace=True),
            nn.Conv2d(8 // 2, 8, kernel_size=1, stride=1, padding=0)
        )
        self.spectral_attention = nn.Sequential(
            nn.Conv2d(8, 8 // 2, kernel_size=1, padding=0),
            nn.ReLU(),
            nn.Conv2d(8 // 2, 8, kernel_size=1, padding=0),
            nn.ReLU()
        )
        ...
        self.decoder2 = nn.Sequential(
            nn.Conv3d(in_channels=1, out_channels=1, kernel_size=(5, 1, 1), stride=1, padding=0),      # 672-4=668
            nn.Conv3d(in_channels=1, out_channels=1, kernel_size=(5, 1, 1), stride=1, padding=0),      # 668-4=664
            nn.Hardtanh(),
            nn.MaxPool3d((2, 1, 1)),      # 664/2=332
            nn.Dropout(0.2),

            nn.Conv3d(in_channels=1, out_channels=1, kernel_size=(5, 1, 1), stride=1, padding=0),      # 332-4=328
            nn.Hardtanh(),
            nn.MaxPool3d((2, 1, 1)),      # 328/2=164
            nn.Dropout(0.2),
        )
        self.upscale2 = nn.Sequential(
            nn.Linear(164, 100*100)
        )
