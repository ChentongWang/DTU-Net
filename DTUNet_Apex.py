...
# Load DATA
data = sio.loadmat("apex_dataset.mat")
...
def plot_abundance(abundance_input, abundance_GT_input):
    ...
    plt.savefig('DNAE_abundances_apex_5.png')
    plt.show()
...
class Dilateformer(nn.Module):
    def __init__(self, img_size=110, patch_size=1, in_chans=285, num_classes=1000, embed_dim=216,
...
class multiStageUnmixing(nn.Module):
    def __init__(self):
        ...
        self.spectral = nn.Sequential(
            nn.Conv3d(in_channels=1, out_channels=1, kernel_size=(4, 1, 1), stride=1,
                      padding=0),
            ...
            ChannelAttention(in_planes=band_Number-3, ratio=2),
            ...
            ChannelAttention(in_planes=band_Number-5, ratio=2),
            ...
            ChannelAttention(in_planes=(band_Number-5)//2-2, ratio=2),
            ...
            nn.LeakyReLU(0.2),
            ChannelAttention(in_planes=((band_Number-5)//2-2)//2-3, ratio=2),
            nn.MaxPool3d((2, 1, 1)),
            ...
            ChannelAttention(in_planes=(((band_Number-5)//2-2)//2-3)//2-3, ratio=2),
            ...
            ChannelAttention(in_planes=((((band_Number-5)//2-2)//2-3)//2-3)//2-3, ratio=2),

            nn.MaxPool3d((2, 1, 1)),

            nn.Dropout(0.1),

            nn.Conv2d(in_channels=(((((band_Number-5)//2-2)//2-3)//2-3)//2-3)//2, out_channels=endmember_number,
                      kernel_size=3, stride=1, padding=1)
        )
        self.upscale = nn.Sequential(
            nn.Linear((C*8)//endmember_number, col ** 2),
        )
...
def reconstruction_SADloss(output, target):
    input_norm = torch.sqrt(torch.bmm(output.view(-1, 1, band_Number),
                                      output.view(-1, band_Number, 1)))
    target_norm = torch.sqrt(torch.bmm(target.view(-1, 1, band_Number),
                                       target.view(-1, band_Number, 1)))

    summation = torch.bmm(output.view(-1, 1, band_Number), target.view(-1, band_Number, 1))
    angle = torch.acos(summation / (input_norm * target_norm))
    return angle


MSE = torch.nn.MSELoss(size_average=True, reduction='mean')
...
def set_optimizer(model, lr_base, decay):
    ...
    optimizer = torch.optim.Adam([
        {'params': model.decoderlayer4.parameters(), 'lr': 0.01, 'weight_decay': 1e-8},
        {'params': else_params}], lr=lr_base, weight_decay=decay
    )
    return optimizer

optimizer = set_optimizer(model=net, lr_base=learning_rate, decay=3e-4)
...
for epoch in range(EPOCH):
    for i, (x, y) in enumerate(train_loader):
        ...
        abundanceLoss = reconstruction_SADloss(reconstruction_result.view(1, band_Number, -1).transpose(1, 2),
                                          x.view(1, band_Number, -1).transpose(1, 2))

        MSELoss = MSE(x, reconstruction_result)

        ALoss = torch.mean(abundanceLoss)
        BLoss = MSELoss
