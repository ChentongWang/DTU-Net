...
class MultiDilatelocalAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        attns = []  # 新增: 创建一个列表来收集注意力图
        for i in range(self.num_dilation):
            # x[i] = self.dilate_attention[i](qkv[i][0], qkv[i][1], qkv[i][2])# B, H, W,C//num_dilation
            x_i, attn_i = self.dilate_attention[i](qkv[i][0], qkv[i][1], qkv[i][2])  # B, H, W,C//num_dilation
            x[i] = x_i  # 修改: 赋值
            attns.append(attn_i)
...
class GlobalBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.permute(0, 2, 3, 1)
        # x = x + self.drop_path(self.attn(self.norm1(x)))
        attn_output, attn_weights = self.attn(self.norm1(x))  # 修改: 接收两个返回值
        x = x + self.drop_path(attn_output)  # 修改: 使用 attn_output
        x = x + self.drop_path(self.mlp(self.norm2(x)))
        x = x.permute(0, 3, 1, 2)
        # return x
        return x, attn_weights
...
class Dilatestage(nn.Module):
    ...
    def forward(self, x):
        ...
        for blk in self.blocks:
            # x = blk(x)
            x, block_attn_weights = blk(x)
            stage_attn_weights.append(block_attn_weights)
        x = self.downsample(x)
        # return x
        return x, stage_attn_weights
...
class Dilateformer(nn.Module):
    def __init__(self, img_size=95, patch_size=1, in_chans=156, num_classes=1000, embed_dim=96,
    ...
    def forward_features(self, x):
        ...
        for i, stage in enumerate(self.stages):
            x, stage_attn_weights = stage(x)
            all_attn_weights[f'stage_{i}'] = stage_attn_weights
    ...
    def forward(self, x):
        ...
        x, all_attn_weights = self.forward_features(x)
        # x = self.head(x)
        # return x
        return x, all_attn_weights
...
class multiStageUnmixing(nn.Module):
    def __init__(self):
        ...
        self.spectral = nn.Sequential(
            ...
            nn.Conv2d(in_channels=((((band_Number-4)//2-2)//2-2)//2-2)//2, out_channels=endmember_number,
    ...
    def forward(self, x):
        cls_emb, all_attn_weights = self.dilate(x)
        ...
        return en_result1, de_result, b, all_attn_weights
...
def set_optimizer(model, lr_base, decay):
    ...
    optimizer = torch.optim.Adam([
        {'params': model.decoderlayer4.parameters(), 'lr': 0.02},
...
for epoch in range(EPOCH):
    for i, (x, y) in enumerate(train_loader):
        ...
        en_abundance, reconstruction_result, b_result, all_attn_weights = net(x)
